import {cfg,save,nextChar,press,reset} from "./accuracy.js";
let running=false,timer=0,start=0,typed=0,next=0,audio=null;
const $=id=>document.getElementById(id);
const status=x=>{const e=$("atStatus");if(e)e.textContent=x};
function sound(){try{audio??=new AudioContext();if(audio.state==="suspended")audio.resume();const o=audio.createOscillator(),g=audio.createGain();o.type="square";o.frequency.value=700;g.gain.setValueAtTime(.025,audio.currentTime);g.gain.exponentialRampToValueAtTime(.001,audio.currentTime+.02);o.connect(g).connect(audio.destination);o.start();o.stop(audio.currentTime+.02)}catch{}}
function stop(){running=false;clearTimeout(timer);timer=0;typed=0;reset();status("Ready")}
function tick(){
 if(!running)return;
 const wpm=Math.max(Number(cfg.wpm)||1,.000001),gap=12000/wpm,now=performance.now();let n=0;
 while(running&&next<=now&&n++<1000){const ch=nextChar();if(ch===null){stop();return}sound();if(!press(ch)){stop();return}typed++;next=start+typed*gap}
 timer=setTimeout(tick,Math.max(1,next-performance.now()));
}
function startTest(){if(!$("wordsInput")){status("Open a typing test first");return}stop();running=true;status("Typing...");start=performance.now();typed=0;next=start+12000/Math.max(Number(cfg.wpm)||1,.000001);tick()}
function gui(){
 if($("autotyper"))return;
 ...
 $("atStart").onclick=startTest;$("atStop").onclick=stop;g.addEventListener("keydown",e=>e.stopPropagation(),true)
}
document.addEventListener("keydown",e=>{const t=e.target,editing=t&&(t.tagName==="INPUT"||t.tagName==="TEXTAREA"||t.isContentEditable);if(e.code==="ArrowRight"&&editing)return;if(e.code==="ArrowRight"&&!e.repeat){e.preventDefault();startTest()}},true);
