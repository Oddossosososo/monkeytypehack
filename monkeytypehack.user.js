...
function stop(){running=false;clearTimeout(timer);timer=null;type.nextTime=0;type.word="";type.pos=0;if($("status"))$("status").textContent="Ready"}
function start(){if(!document.querySelector("#typingTest")||!$("wordsInput"))return;running=true;if($("status"))$("status").textContent="Typing...";type.nextTime=performance.now();type.word="";type.pos=0;type()}
...
function nextWord(){const w=document.querySelector(".word.active");return w?[...w.children].map(x=>x.textContent).join(""):""}
function nextChar(){const w=document.querySelector(".word.active"),word=nextWord();if(!w)return null;if(type.word!==word){type.word=word;type.pos=0}return type.pos<word.length?word[type.pos++]:" "}
...
function type(){if(!running)return;const wpm=Math.max(Number(cfg.wpm)||1,.000001),ms=12000/wpm,now=performance.now();if(!type.nextTime)type.nextTime=now;let n=0;while(running&&type.nextTime<=now&&n++<500){const ch=nextChar();if(!ch)return stop();press(ch);type.nextTime+=ms}if(running)timer=setTimeout(type,Math.max(0,type.nextTime-performance.now(),1))}
