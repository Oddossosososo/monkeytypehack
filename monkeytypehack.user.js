...
function type(){if(!running)return;const ch=nextChar();if(!ch)return stop();press(ch);const wpm=Math.max(Number(cfg.wpm)||1,.000001),ms=12000/wpm;if(!type.nextTime)type.nextTime=performance.now();type.nextTime+=ms;timer=setTimeout(type,Math.max(0,type.nextTime-performance.now()))}
