...
function press(key){const input=$("wordsInput");if(!input)return;const code=key===" "?"Space":/^[a-z]$/i.test(key)?"Key"+key.toUpperCase():"";input.dispatchEvent(new KeyboardEvent("keydown",{key,code,bubbles:true,cancelable:true}));input.value=input.value+key;input.dispatchEvent(new InputEvent("input",{inputType:"insertText",data:key,bubbles:true,cancelable:true}));input.dispatchEvent(new KeyboardEvent("keyup",{key,code,bubbles:true,cancelable:true}))}
...
$("accuracy").addEventListener("input",e=>{let v=parseFloat(e.target.value);if(Number.isFinite(v)){v=Math.min(1,Math.max(0,v));cfg.accuracy=v;save()}});
