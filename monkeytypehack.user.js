...
function press(key){const input=$("wordsInput");if(!input)return;const code=key===" "?"Space":/^[a-z]$/i.test(key)?"Key"+key.toUpperCase():"";input.dispatchEvent(new KeyboardEvent("keydown",{key,code,bubbles:true,cancelable:true}));input.dispatchEvent(new KeyboardEvent("keyup",{key,code,bubbles:true,cancelable:true}))}
